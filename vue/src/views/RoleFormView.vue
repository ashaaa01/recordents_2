...
if (route.params.id) {
  store.dispatch("editRole", route.params.id);
  addForm.value = false;
  formTitle = 'Update';
}
...
const selected = computed(() => store.state.selectedRole);

// next

import RoleEditor from "@/mycomponents/editor/RoleEditor.vue";
...
function submit() {
 ...
 model.value.services.forEach((service) => {
    service.permission_data.push({ id: 100});
  });


 if (route.params.id) {
  store.dispatch("updateRole", { ...model.value }).then(({ data }) => {
    successAlert();
    router.push({
      name: `${props.role}-roles`
    });
  })
  .catch(err => {
    dangerAlert(err.response.data.errors);
  });
} else {
  store.dispatch("saveRole", { ...model.value }).then(({ data }) => {
    successAlert();
    router.push({
      name: `${props.role}-roles`
    });
      ...
      <SectionTitleLineWithButton :icon="mdiPlusBox " :title="route.meta.title" main>
        ...
        <CardBox is-form @submit.prevent="submit">
          <div class="px-4 py-5 space-y-6 sm:p-6">
            <div v-if="addForm" >
              <h3 class="text-2xl font-semibold flex items-center justify-end dark:text-slate-300">
              ...
              <div v-if="!model.services.length" class="text-center text-gray-600 dark:text-slate-300">
                You don't have any role created
              </div>
            </div>
