...
const items = computed(() => store.state.userList.filter(user => user.role_id === props.roleID));
...
<template>
  <CardBoxModal v-if="selectedRecord.value != null" v-model="isModalActive" title="User Details" classValue="flex overflow-x-auto shadow-lg max-h-modal w-11/12 md:w-3/5 lg:w-3/5 xl:w-6/12 z-50">
    <SelectedUserCard :user="selectedRecord" />
  </CardBoxModal>
      ...
      <tr v-for="users in usersPaginated" :key="users.id">
        ...
        <td class="before:hidden lg:w-1 whitespace-nowrap">
          <BaseButtons type="justify-start lg:justify-end" no-wrap>
            ...
            <BaseButton color="success" :icon="mdiAccountEdit" small :to="`/${role}/edit-user/${users.id}`" />
          </BaseButtons>
        </td>
      </tr>
    </tbody>
